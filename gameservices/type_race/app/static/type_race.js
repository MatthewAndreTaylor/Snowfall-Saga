...
socket.addEventListener('open', (event) => {
    socket.send(JSON.stringify({ type: 'username', username: sessionStorage.getItem('username') }));
});
...
function handleKey(e) {
  ...
  if (/^[a-zA-Z\s,.!?'":;]$/.test(event.key)) {
    ...
    if (inputtedString[i] == text[i]) {
        characters[i] = '<span style="color: #7fffd4;">' + characters[i] + '</span>';
        if (i == totalCount) {
            if (!done) {
                correct = correct + 1;
            }
...
function updateServer() {
    socket.send(JSON.stringify({ type: 'playerUpdate', correct: correct, incorrect: incorrect }));
}
...
function slideText() {
    let slice = characters.slice(Math.max(0, i - 5), Math.min(characters.length, i + 105));
...
//  divBar.innerText = percentage + "%"; // Display progress percentage
  ...
  var reindeerPosition = (percentage / 100) * divProgressWidth - imgReindeerWidth;
